...
/*统计server port klh*/
int serverPort_statistics(unsigned portid)
{
    int i,pos,key;
...
print_stats(void)
{
    ...
    for (portid = 0; portid < RTE_MAX_ETHPORTS; portid++) {
        ...
        for (i = 0; i < 20; i++){
            printf("%d.%d.%d.%d\t\t%lu\n",f20_bytes_serverIP[i].server_ip[0],f20_bytes_serverIP[i].server_ip[1],f20_bytes_serverIP[i].server_ip[2],f20_serverIP[i].server_ip[3],f20_bytes_serverIP[i].total_bytes);
        }
        /*清零f20_serverIP计数*/
